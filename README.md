...
474 座開在真實位置、掛著真實編號的出入口、20 座轉乘站的轉乘通道、
...
187 座站體共 **474 座折返式樓梯井**（含複合體裡地下街接不上、退回這裡各自接進
所屬站體的 7 座），每一座門口都立著「出口 N／站名／Exit N」；資料裡沒有出入口的
35 座站體（機場捷運桃園段、安坑輕軌、淡海輕軌的平面站）用一個預設位置的出入口
走同一套流程。
...
接不上的 24 個各有理由，報表會印：撞到別的井或通道 12（其中 4 個是複合體
退回來的）、街面與穿堂同高 5（高架站，見下）、通道會露出地面 4、撞到別線的
結構 2（北投的兩個，正下方是新北投支線）、離站體 350 m 以上 1
（忠孝新生的新生南路緊急出口）。
...
高架站多出來的一種接不上：**街面與穿堂同高**（木柵、東湖、淡江大學、淡金鄧公、
鶯歌車站各 1 個）。橋下穿堂只比地面高 3～7 m，山坡上的出入口街面可能就在
穿堂那個高度前後兩公尺內，井的門洞會矮到鑽不過去，那幾個先跳過。

### 轉乘通道

20 座轉乘站（台北車站複合體以外）的兩座站體之間接一條**付費區對付費區**的
...
全網 474 座裡 473 座走得到月台、473 座門開在街上，22 座轉乘站的出入口
各自全在同一團裡（沒過的一座見「已知限制」）。
...
- 474 座出入口以外還有 24 個接不上（理由見「真實出入口」，其中 5 個是
  高架站街面與穿堂同高、4 個是複合體裡地下街也接不上的）。
  蓋出來的 474 座裡 `verify_exits` 有 1 座沒過：淡江大學的預設出入口，
  街面只比橋下穿堂低 3 m，井蓋出來門洞只有兩格、門口也踩不到街，還沒查
