...
**出入口**用 OSM 的實際座標（M1~M8、K、Z 系列）。每個是一座折返式樓梯井
（`ShaftStair`）加一條水平通道（`Passage`）接到對應的穿堂層。
折返梯是必要的：真實出入口離站體常常不到 40 m，但深 39 m 的穿堂需要
78 m 的直線長梯，硬拉會穿到別人家。折返把 39 m 摺進 20×14 m 的井裡，
井口有頂蓋與門（第一版沒有，是個沒蓋子的陷阱）。
