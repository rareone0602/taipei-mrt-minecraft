...
用法:
  python scripts/slice_world.py 台北車站            # 垂直橫剖面（垂直於路線）
  python scripts/slice_world.py 台北車站 --long     # 縱剖面（沿路線）
  python scripts/slice_world.py --xz 0 0 --dir 1 0
"""
import io, os, re, sys, glob, json, csv, math, zlib, argparse
import numpy as np, nbtlib

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SAVE = os.path.expanduser("~/Library/Application Support/minecraft/saves/Taipei MRT")
...
def main():
    ...
    rd = Reader(os.path.join(a.save, "dimensions/minecraft/overworld/region")
                if a.save else RDIR)
    # 先掃出這條剖面上有東西的 y 範圍
    cols = []
