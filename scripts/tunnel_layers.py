...
用法（分析報告）: ./.venv/bin/python scripts/tunnel_layers.py
"""
...
def station_pins(min_margin=15.0, ratio=2.0, radius=120.0, verbose=False):
    ...
    pf = os.path.join(BL.ROOT, "data", "platform_levels.json")
    if not os.path.exists(pf):
        return []
    items = json.load(open(pf, encoding="utf-8"))["items"]
    lines = json.load(open(os.path.join(BL.ROOT, "data", "mc_lines.json"),
                           encoding="utf-8"))
