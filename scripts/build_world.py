...
用法: ./.venv/bin/python scripts/build_world.py [--corridor 160] [--lines BR ...]
"""
import sys, os, json, csv, math, argparse, shutil, time
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import numpy as np
import mcworld
from mcworld import World, Chunk
from terrain import Terrain, SEA_Y
import build_line as BL
import rails
import tunnel_layers as TL
...
def terrain_chunk(ch, cx, cz, terr, blend, rx, rz):
    ...
    for sy in range(mcworld.SEC_MIN, mcworld.SEC_MAX + 1):
...
def main():
    ...
    w = World(a.out, name="Taipei MRT", spawn=(0, gy + 2, 0))
    w._write_level()
