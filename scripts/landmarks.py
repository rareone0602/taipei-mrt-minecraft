...
World.set() 會自動丟掉不屬於目前 region 的方塊，所以這裡只要對每個
bbox 涵蓋到的 region 各呼叫一次 build()，裁切交給它處理。

自我測試: ./.venv/bin/python scripts/landmarks.py
"""
...
def _test():
    ...
    w = _FakeWorld()
    b.build(w)
    ys = [y for _, y, _ in w.b]
    chk(f"寫出 {len(w.b):,} 個方塊", len(w.b) > 1000)
    ...
    corner = w.b.get((-20, 72, -15))
    chk(f"外牆角 (-20,72,-15) = {corner}", corner not in (None, AIR))

    # 樓板是實的
    chk("2 樓樓板實心", w.b.get((0, 75, 0)) not in (None, AIR))
    ...
    w2 = _FakeWorld()
    s.build(w2)
    chk(f"寫出 {len(w2.b):,} 個方塊", len(w2.b) > 500)
    chk("樓板實心", w2.b.get((0, 50, 0)) not in (None, AIR))
    chk("淨空是空氣", w2.b.get((0, 52, 0)) == AIR)
    chk("頂板實心", w2.b.get((0, 55, 0)) not in (None, AIR))
    chk("周牆封閉", w2.b.get((-10, 52, 0)) == "minecraft:gray_concrete")

    print("退化輸入")
    w3 = _FakeWorld()
    Building([(0, 0), (1, 1)], g0=70).build(w3)
    chk("兩點多邊形不炸且不寫東西", len(w3.b) == 0)
